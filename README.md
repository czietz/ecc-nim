...
let secretA = ecHMACSharedSecret(bobKeys.public, aliceKeys.private, info = "session-id")
let secretB = ecHMACSharedSecret(aliceKeys.public, bobKeys.private, info = "session-id")
...
A supplementary package `sshcompat` is provided that adds support for OpenSSH-compatible public and private keys and signatures in `ecdsa-sha2-nistp256` format. [See the documentation inside the file](https://czietz.github.io/ecc-nim/sshcompat.html) for a description of functions and limitations.
